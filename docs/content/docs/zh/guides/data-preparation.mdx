...
`datasets/` 下的转换器使用与 engine 无关的独立 `dataset-prep` extra：
...
`datasets/droid100/prepare_droid100.py` 还会 import torch；应在某个 engine 环境中运行，
或为仅 CPU 的数据准备显式安装 torch。
...
data-source driver 当前接受以下 `(modality, role)` 组合：

| 组合 | 含义 |
|---|---|
| `image`, `condition` | 编辑或图像条件生成的源图像。 |
| `video`, `condition` | 视频到视频生成的源视频。 |
| `image`, `prompt` | 作为 prompt media 传入的图像。 |
| `audio`, `prompt` | 作为 prompt media 传入的音频。 |
| `video`, `prompt` | 作为 prompt media 传入的视频。 |

例如，通过 `media_refs` 附加条件图像：
...
- 相对 URI 相对数据文件所在目录解析。
- 支持绝对本地路径与 `http://` / `https://` URI。当前会拒绝 `s3://` 和 `gs://`
  引用；请先落到本地或通过 HTTP(S) 提供。
- 每条 prompt 最多一个 image condition，也最多一个 video condition。
- condition batch 必须同质：只要某一行有 image condition，同一 batch 的每一行都必须有；
  video condition 同理。
- 同一 batch 不能混用 `(video, prompt)` 与 `(video, condition)`，请拆成不同 request。
- prompt role media 会成为 batch 对齐、允许稀疏行的 `MediaRefs` primitive，并传给支持它的
  model adapter。data source 接受某个组合，不代表每个 rollout engine/model 都支持它。
...
SFT 与 agentic 数据有专用转换器和 canonical recipe，分别见
`datasets/sft_manifests/`、`datasets/searchgen/` 与 `datasets/asearcher/` 的 README；
其中给出了准确的转换命令以及 `train_sft` / `train_agentic` 启动方式。
