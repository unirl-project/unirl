...
启动 Ray 作业前，先做 Hydra compose check。`--cfg job --resolve` 检查 Hydra 组合、
override 语法以及插值/环境变量解析；它不会实例化运行时 dataclass，也不会运行 trainer
或跨组件校验。
...
单机通用启动器的第一个参数是 `examples/` 下的 recipe 路径
（`<path-under-examples>`，不含 `.yaml`）：
