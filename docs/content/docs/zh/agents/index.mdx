...
- 按 recipe 所属训练域选择入口：`train_diffusion`、`train_ar`、`train_sft`、
  `train_async_diffusion`、`train_async_ar`、`train_pe`、
  `train_unified_model` 或 `train_agentic`。
- `examples/<path-under-examples>.yaml` 文件是权威配置面；路径可能包含模型子目录。
...
| 修改 SFT | `unirl/trainer/sft.py`，然后看对应的 `examples/sft/` recipe |
| 修改 agentic tool use | `unirl/trainer/agentic.py`，然后看 `examples/deep_research/` |
| 修改 async rollout | `unirl/trainer/async_rollout.py`，然后看对应的 async recipe |
