...
Start by composing a recipe before launching Ray work. `--cfg job --resolve`
checks Hydra composition, override syntax, and interpolation/environment
resolution. It does not instantiate runtime dataclasses or run trainer and
...
Use the generic single-node launcher when you want the scripts to prepare local Ray runtime defaults. The first argument is the recipe path below `examples/` (`<path-under-examples>`, without `.yaml`):
