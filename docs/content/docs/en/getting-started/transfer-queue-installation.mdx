...
debugging: `transfer_queue.device_name=mlx5_0`.

`protocol: tcp` is accepted by `MooncakeBackendConfig`, but it is **not** a
no-RDMA-host fallback in the current UniRL runtime. Mooncake client creation
unconditionally probes `/sys/class/infiniband` and fills `device_name` before
initializing the storage manager, regardless of `protocol`. Therefore, setting
only `transfer_queue.protocol=tcp` on a host without an HCA still raises
*"no InfiniBand device found"*. Use `SimpleBackend` for functional testing on
such hosts; validate Mooncake only on an RDMA-capable host until the runtime
gates HCA discovery by protocol.
...
| `no InfiniBand device found under /sys/class/infiniband` | Current Mooncake client setup requires HCA discovery even with `protocol=tcp`; run it on an RDMA host or use `SimpleBackend`. |
