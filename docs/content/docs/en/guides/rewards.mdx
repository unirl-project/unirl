...
A reward is exactly one **backend**, held by `RewardService`
(`unirl/reward/service.py`). `score_and_attach(sample)` scores the generated
primitives on the frontier `Part` and returns the `Sample` with
`Part.rewards` and `Part.component_rewards` attached:
