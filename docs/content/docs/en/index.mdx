...
Each domain has its own entrypoint and trainer lifecycle; the invocation syntax
is consistent:

```bash
python -m unirl.train_diffusion       --config-name=<path-under-examples>  # diffusion image/video
python -m unirl.train_ar              --config-name=<path-under-examples>  # autoregressive/VLM RL
python -m unirl.train_sft             --config-name=<path-under-examples>  # supervised fine-tuning
python -m unirl.train_async_ar        --config-name=<path-under-examples>  # asynchronous AR RL
python -m unirl.train_async_diffusion --config-name=<path-under-examples>  # asynchronous diffusion RL
python -m unirl.train_pe              --config-name=<path-under-examples>  # prompt enhancement
python -m unirl.train_unified_model   --config-name=<path-under-examples>  # mixed AR + diffusion
python -m unirl.train_agentic         --config-name=<path-under-examples>  # agentic tool use
```

`<path-under-examples>` is the selected self-contained YAML's path below
`examples/`, without `.yaml` — for example:
...
Shell launchers in `examples/` should stay thin — they prepare environment variables, start Ray, and pass the recipe name plus Hydra overrides, while the recipe semantics live in YAML. See `examples/run_experiment_single_node.sh` and `examples/run_experiment_multinode.sh` for the canonical pattern.
