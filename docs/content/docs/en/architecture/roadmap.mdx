...
- **Infra** — harden the pluggable native FSDP/HSDP and VeOmni FSDP/SP/EP paths,
  vLLM-Omni rollout, async reward, and cross-engine conformance, and add a
...
|---|---|---|---|
| Infra | VeOmni training backend | `P0` | `[x]` done |
...
- **Baseline.** The trainer exposes a swappable `backend:` block. Native
  `FSDPBackend` lives in `unirl/train/backend/fsdp/backend.py` and supports
  FSDP2, HSDP, and no-shard modes; `VeOmniBackend` lives under
  `unirl/train/backend/veomni/` and supplies FSDP2, sequence-parallel, and
  expert-parallel paths for checked-in recipes.
- **Goal.** Keep both backends behind the same train-stack contract and extend
  model coverage without weakening topology validation.
- **Tracking.** *(RFC needed)*
- TODO:
  - `[x]` Implement `VeOmniBackend` and checked-in SP/EP recipes.
  - `[x]` Make native HSDP shard size explicit through `FSDPConfig`.
  - `[ ]` Torch Distributed Checkpoint and resume parity with `FSDPBackend`.
  - `[ ]` Extend model-specific EP export layouts and weight-sync coverage.

### `[ ]` `P0` REFL Support (differentiable-reward training mode)

- **Baseline.** Rewards are computed as scalars and attached to the frontier `Part`, then
  turned into advantages by `Part.compute_advantages`. There is no gradient path from a reward
  model back into the denoiser. Train-side algorithms today include diffusion policy-gradient
  (`FlowGRPO` / `FlowDPPO`), autoregressive policy-gradient (`GRPO` and related objectives),
  and forward-process (`DiffusionNFT`) methods.
...
- **Baseline.** The generated `Part`'s sampling params treat sigmas as the single source of
  truth across trainside / SGLang / vLLM-Omni, and
  `rollout/engine/sigma_verify.py` provides validation.
...
|---|---|---|---|
| Policy-Gradient / PPO | rollout log-probs + advantages through a clipped / KL surrogate | FlowGRPO (including DanceGRPO / MixGRPO recipes), GRPO, GSPO, PPO, DPPO / CPPO, FlowDPPO, DRPO | DDPO, DPOK, KL / reference policy |
...
- **Baseline.** PPO-style objectives are already in use: `FlowGRPO`
  (`flowgrpo.py`) is the diffusion/flow clipped-ratio objective, `GRPO`
  (`grpo.py`) is the text / VLM objective, and `FlowDPPO` provides diffusion
  KL-ADV masking. AR `PPO` already includes a value head and GAE; the remaining
  gap here is diffusion-native PPO recipes and broader control variants.
- **Tracking.** *(RFC needed)*
- Planned types:
  - `[x]` **AR PPO (value critic + GAE)** — implemented by `PPO`; diffusion
    value-critic support remains open.
...
- **Goal.** Provide shared reference-policy infrastructure and KL controllers for PPO, DPOK,
  Diffusion-DPO, GRPO / GSPO, and related methods.
- **Tracking.** *(RFC needed)*
- TODO:
...
- **Baseline.** `Part.compute_advantages` already supports GRPO-style group advantages, while
  `Sample.propagate_rewards` supports mean / max / sum parent-child reward aggregation.
  Shared numeric helpers live in `types/advantages.py`.
...
- *Candidate.* 🙋 Diffusion-DPO — pairwise preference optimization (no reward model).
  Note this is distinct from the existing `FlowDPPO`, which is policy optimization with
