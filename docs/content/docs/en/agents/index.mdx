...
- Use the training-domain entrypoint that owns the recipe: `train_diffusion`,
  `train_ar`, `train_sft`, `train_async_diffusion`, `train_async_ar`,
  `train_pe`, `train_unified_model`, or `train_agentic`.
- Treat `examples/<path-under-examples>.yaml` files as the authoritative configuration surface; paths may include model subdirectories.
...
| Work on SFT | `unirl/trainer/sft.py`, then the matching `examples/sft/` recipe |
| Work on agentic tool use | `unirl/trainer/agentic.py`, then `examples/deep_research/` |
| Work on async rollout | `unirl/trainer/async_rollout.py`, then the matching async recipe |
